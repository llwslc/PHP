...
function gst()
{
	...
	spt(64, 59, 266);
	/* 购买道具 */
	ctx.drawImage(img7, 320, 320, 202, 57, 59, 333, 202, 57);
...
function init()
{
    ...
    canvas.style.left = Math.floor((ww - canvas.width) / 2) + "px";
	...
	askb = 1; - 1 < document.URL.indexOf("page") ?
		(e = ["title_jmp_nolink.jpg", "cs_jmp.gif", "cs_jmp_bg.jpg", "cs_jmp_nolink.png"],
			canvas.style.backgroundImage = "url("+$('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/title_jmp_nolink.jpg)",
		...
		(e = ["title_jmp.jpg", "cs_jmp.gif", "cs_jmp_bg.jpg", "cs_jmp.png"],
			canvas.style.backgroundImage = "url("+$('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/title_jmp.jpg)",
			shms1 = "Browser game[SKY JUMP] SCORE=",
			shms2 = "Browser game[SKY JUMP]",
			shms3 = "Why don't you get useful tips by sharing this game to your friends?",
			shms4 = "Why don't you get useful items by sharing this game to your friends?");
	img0 = new Image;
	img0.src = $('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/" + e[0];
	img0.onload = function()
	{
		gsts()
	};
	img1 = new Image;
	img1.src = $('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/" + e[1];
	img1.onload = function()
	{
		gsts()
	};
	img2 = new Image;
	img2.src = $('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/" + e[2];
	img2.onload = function()
	{
		gsts()
	};
	img7 = new Image;
	img7.src = $('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/" + e[3];
	img7.onload = function()
	{
		gsts()
	};
	imgShop = new Image;
	imgShop.src = $('#publicUrl').attr('paramUrl')+"/Rangwofeiyihui/skj/shop.png";
	imgShop.onload = function()
	{
		gsts()
	};
	bx = [];
...
function Press(e)
{
	...
	switch (s)
	{
		...
		case 0:
			/* 主界面 */
			e = e.y / wdpr | 0;
			333 < e ? onShopSel() : (gmstini(), 6 != dvid ? stm = setTimeout("lp()", 1e3 / stti2) : (window.requestAnimationFrame = function()
			{
...
function onShopSel()
{
	...
	rlok = 1;
	/* 画道具 */
	ctx.drawImage(imgShop, 0, 0, 292, 73, 14, 90, 292, 73);
	ctx.drawImage(imgShop, 0, 73, 292, 73, 14, 173, 292, 73);
	ctx.drawImage(imgShop, 0, 146, 292, 73, 14, 256, 292, 73);
	ctx.drawImage(imgShop, 195, 219, 94, 49, 14, 346, 94, 49);
	setTimeout(scrollTo, 200, 0, 1);
}
